...
/**
 * win_minmax.c: windowed min/max tracker
 *
			 ...
			 const struct minmax_sample *val)
{
	...
	if (unlikely(dt > win)) {
		 ...
		 * Passed entire window without a new val so make 2nd
		 * choice the new val & 3rd choice the new 2nd choice,
		 * unless 2nd choice is also outside window, in which
		 * case make 3rd choice the new val and 2nd and 3rd
		 * choice (we checked on entry that the third choice
		 * was in the window).
		 */
		if (unlikely(val->t - m->s[1].t > win)) {
			m->s[0] = m->s[2];
			m->s[1] = *val;
		} else {
			m->s[0] = m->s[1];
